...
2. **모델링**
   ...
   **분석유형은 크게 아래와 같이 구분됨**
   ...
   2) **연관성분석** (혐의설비 도출), **추천**, **소셜빅데이터**, **군집화**
   3) **수치예측**(품질예측, 기업신용평가, 구매단가예측, 적정재고산출), **수요예측**(판매수량예측, 거점별수요예측), **분류**(불량 여부 판정, 대포통장적발, 거래유형 자동분류, 퇴사자예측, 채권 사고확률예측, 마케팅 target선정, 멤버십고객 해지방어, VoC유형분류, 구매item 유형분류), **영향인자발굴**(품질영향인자 발굴), **최적조건도출**(설비운영조건 도출, 생산계획 최적화, 에너지 관리 최적화, 마케팅 자원배분 최적화) 
    - 모두 지도학습 적용
