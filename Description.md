...
2. **모델링**
   ...
   **예측 및 분류 모델일 경우**
   ...
   - (선형모델일 경우) tree 기반의 모델 사용, 상관성이 높은 변수1개, PCA사용
   - train / test 분리
  ...
  **연관성 분석 - 혐의설비 도출인 경우**
   ...
   **원인탐지(분석) / 최적 운영조건 도출 / 영향인자 발굴인 경우**
   ...
   <영향인자 발굴>
   - 영향인자 발굴은 모니터링, 상황 control, 작업의 우선순위 선정, 상황의 해석을 위해 사용함
   - 변수 선택 (전진선택법, 후진소거법, 단계선택법) -> 영향력 판단 기준: 유의 확률, aic or bic
   - tree 계열 모델의 변수 중요도
   - 주성분 분석 활용 (y라벨을 잘 구별할 수 있는 주성분 z1의 X1~Xn 변수 가중치를 확인하여 변수 선택 가능   
   - 유용성 검증은?
   : 지도학습 모델의 예측력 확보 / 일관성 확보 (shap explanation force plot)   
   <최적조건 도출(불량을 최소화하는 최적 조건 도출 방법)>   
   - 의사결정나무   
   : 불량률이 가장 작은 그룹의 분류 기준을 최적 조건으로 제안 가능함   
